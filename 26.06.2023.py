...
# Если хотим добавить какую-либо инф-ю в файл, необходимо использовать режим 'a' - append:
file = open('teest.txt', 'a')
file.write(' TEST')
file.close()

# Режим 'b' - Binary mode нужен для того, чтобы считывать с помощью Python какие-нибудь изображения,
# музыкальные файлы, видео файлы, медио контент или что-нибудь, что требует бинарный доступ.